...
func (w *Worker) Start(ctx context.Context) {
	log.Printf("Starting VartaCore background news worker (Interval: %v)", w.cfg.NewsFetchInterval)
	...
	go func() {
		...
		for {
			select {
			case <-ctx.Done():
				log.Println("Stopping VartaCore news worker...")
				return
			case <-ticker.C:
				w.RunPipeline(ctx)
			}
...
func (w *Worker) RunPipeline(ctx context.Context) {
	log.Println("Starting scheduled news ingestion cycle...")
	newArticlesCount := 0
	...
	if w.cfg.NewsAPIKey != "" {
		...
		if err != nil {
			log.Printf("Error fetching News API: %v", err)
		} else {
			newArticlesCount += count
		}
	}

	log.Printf("Ingestion cycle complete. Stored %d new positive articles.", newArticlesCount)
	...
	if newArticlesCount > 0 {
		...
		log.Println("Refreshed article and category caches")
	}
}
